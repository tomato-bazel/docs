...
description: Publish a versioned module to the registry with the rels tool.
editPath: authoring-a-module.md
---

Modules are published to the registry with the `rels` tool (in [`tomato-bazel/bazel-registry`](https://github.com/tomato-bazel/bazel-registry)), which cuts a version from a GitHub tag.
...
3. Commit + push. `registry.tbzl.dev` serves the new version within about a minute — and it appears in the [Modules reference](/reference/modules/) on the next docs build.
